...
static long long do_float_output(struct fmt* spec, char** dest, size_t* dsize, union arg* arg) {
	...
	// Call dtostr
	int result =
	    dtostr(conversion, arg->d, (spec->precision < 0 ? 6 : spec->precision), sizeof(conversion));
	if (result < 0) {
		return -1;
	}
...
static int do_int(char* dest, char qualifier, unsigned long long x, unsigned int base,
                  size_t dsize) {
