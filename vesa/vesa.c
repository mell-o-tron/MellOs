...
void fb_draw_line_at_only(int x1, int y1, int x2, int y2, size_t thickness, VESA_Colour col,
                          Framebuffer* fb, Recti bounds) {
	int dx = x2 - x1;
	int dy = y2 - y1;
...
void fb_draw_rect_at_only(int x, int y, size_t width, size_t height, size_t thickness,
                          VESA_Colour col, Framebuffer* fb, Recti bounds) {
...
void fb_draw_rect(int x, int y, size_t width, size_t height, size_t thickness, VESA_Colour col,
                  Framebuffer* fb) {
...
void fb_fill_rect_at_only(int x, int y, int width, int height, VESA_Colour col, Framebuffer* fb,
                          Recti bounds) {
	if (x < 0) {
		width += x;
		x = 0;
...
void fb_draw_char(uint16_t x, uint16_t y, char c, VESA_Colour colour, float scaleX, float scaleY,
                  Framebuffer* fb) {
...
void fb_draw_string(uint16_t x, uint16_t y, const char* s, VESA_Colour colour, float scaleX,
                    float scaleY, Framebuffer* fb) {
	while (*s) {
...
Framebuffer* allocate_full_screen_framebuffer() {
	return allocate_framebuffer(Hres, Vres, CONFIG_GFX_BPP);
}
...
void _blit(Framebuffer* srcptr, Framebuffer* destptr, int x, int y, uint32_t width, uint32_t height,
	...
	if (width < 0 || height < 0 || x > ((int)destptr->width) || y > ((int)destptr->height))
		return;
	if (from_x > x + srcptr->width || from_x > destptr->width || from_y > y + srcptr->height ||
	    from_y > destptr->height)
		return;
	if (from_x < 0)
		from_x = 0;
	if (from_y < 0)
		from_y = 0;
	if (to_x > destptr->width)
		to_x = destptr->width;
	if (to_y > destptr->height)
		to_y = destptr->height;
...
void blit_all_at_only(Framebuffer* src, Framebuffer* dest, int x, int y, int from_x, int from_y,
                      int to_x, int to_y) {
...
void blit_all_at_only_square(Framebuffer* src, Framebuffer* dest, int x, int y, Recti square,
                             int width) {
	int half = width / 2;
	...
	for (int i = -half; i <= half; ++i) {
		...
		// Top
		blit_clamped(src, dest, square.x - half, top, square.x + square.width + half, top + 1);

		// Bottom
		blit_clamped(src, dest, square.x - half, bottom - 1, square.x + square.width + half,
		             bottom);

		// Left
		blit_clamped(src, dest, left, square.y + half, left + 1, square.y + square.width - half);

		// Right
		blit_clamped(src, dest, right - 1, square.y - half, right, square.y + square.width - half);
	}
}

void fb_draw_gradient_at_only(int x, int y, int width, int height, VESA_Colour col1,
                              VESA_Colour col2, Framebuffer* fb, Recti bounds) {
...
void fb_draw_gradient(int x, int y, int width, int height, VESA_Colour col1, VESA_Colour col2,
                      Framebuffer* fb) {
