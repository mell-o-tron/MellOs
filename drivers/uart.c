...
void uart_print_all(const char* s) {
    if (enabled_ports.com1) {uart_print(s, COM1);}
    if (enabled_ports.com2) {uart_print(s, COM2);}
    if (enabled_ports.com3) {uart_print(s, COM3);}
    if (enabled_ports.com4) {uart_print(s, COM4);}
}

char read_any_serial_non_blocking() {
    char out = 0;
    if (enabled_ports.com1) {out = read_serial_non_blocking(COM1);}
    if (out != 0) return out;
    if (enabled_ports.com2) {out = read_serial_non_blocking(COM2);}
    if (out != 0) return out;
    if (enabled_ports.com3) {out = read_serial_non_blocking(COM3);}
    if (out != 0) return out;
    if (enabled_ports.com4) {out = read_serial_non_blocking(COM4);}
    return out;
}