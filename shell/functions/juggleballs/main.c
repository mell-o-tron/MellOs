...
Vec3 ray_sphere_hit_normal (Ray*, Vec3, Sphere*);
Vec3 vec3_sum (Vec3, Vec3);
void fb_set_pixel (size_t, size_t, VESA_Colour, Framebuffer*);
void juggle_balls (char*);
VESA_Colour cast_ray (Ray*, Sphere_3_arr, Parallelepiped_0_arr, int);
Vec3_opt ray_sphere_intersection_point (Ray*, Sphere*);
VESA_Colour make_VESA_Colour (int, int, int, int);
size_t get_width (Window*);
Vec3 ray_parallelepiped_hit_normal (Ray*, Vec3, Parallelepiped*);
...
Vec3_opt ray_sphere_intersection_point(Ray* ray, Sphere* sphere) {
    ...
    Vec3 oc = vec3_sum((*ray).origin, vec3_scale((*sphere).position, -1.));
...
Vec3 ray_sphere_hit_normal(Ray* ray, Vec3 hit_point, Sphere* sphere) {
    Vec3 normal = vec3_scale(vec3_sum(hit_point, vec3_scale((*sphere).position, -1.)), ((float)(1.)) / ((float)((*sphere).radius)));
    return (vec3_dot(normal, (*ray).direction) > 0. ? ((Vec3){0., 0., 0.}) : normal);
}

Ray camera_get_ray(PerspectiveCamera* cam, float u, float v) {
    Vec3 forward = vec3_sum((*cam).lookat, vec3_scale((*cam).eye, -1.));
    Vec3 right = vec3_cross(forward, (*cam).up);
    Vec3 up = (*cam).up;
    Vec3 direction = vec3_normalized(vec3_sum(vec3_sum(vec3_scale(right, u), vec3_scale(up, v)), forward));
...
Vec3 ray_parallelepiped_hit_normal(Ray* ray, Vec3 hit_point, Parallelepiped* para) {
...
uint32_t hres = ((uint32_t)(400));

uint32_t vres = ((uint32_t)(400));
...
VESA_Colour cast_ray(Ray* ray, Sphere_3_arr spheres, Parallelepiped_0_arr parallelepipeds, int max_bounces) {
    if (max_bounces <= 0) {
    ...
    for (size_t i = ((size_t)(0)); i < 3; i = ((size_t)(i)) + ((size_t)(1))) {
        ...
        if (hit.is_some) {
            Vec3 normal = ray_sphere_hit_normal(ray, ((Vec3)hit.contents), &spheres[((int)(i))]);
            return make_VESA_Colour(((int)(((float)(((float)((normal.x))) * ((float)(0.5)))) * ((float)(255.)))), ((int)(((float)(((float)((normal.y))) * ((float)(0.5)))) * ((float)(255.)))), ((int)(((float)(((float)((normal.z))) * ((float)(0.5)))) * ((float)(255.)))), 255);
        } else {
    ...
    for (size_t j = ((size_t)(0)); j < 0; j = ((size_t)(j)) + ((size_t)(1))) {
        ...
        if (hit.is_some) {
            Vec3 normal = ray_parallelepiped_hit_normal(ray, ((Vec3)hit.contents), &parallelepipeds[((int)(j))]);
            return make_VESA_Colour(((int)(((float)(((float)((normal.x))) * ((float)(0.5)))) * ((float)(255.)))), ((int)(((float)(((float)((normal.y))) * ((float)(0.5)))) * ((float)(255.)))), ((int)(((float)(((float)((normal.z))) * ((float)(0.5)))) * ((float)(255.)))), 255);
        } else {
...
void juggle(Sphere* sphere, int ticks) {
...
void juggle_balls(char* s) {
    ...
    Window* w = create_window_with_size("Juggling...", hres, vres);
    set_window_dirty(w);
    fb_draw_rect(10, 10, ((size_t)(get_width(w))) - ((size_t)(((size_t)(10)))), ((size_t)(get_height(w))) - ((size_t)(((size_t)(10)))), ((size_t)(1)), make_VESA_Colour(255, 255, 255, 255), get_fb(w));
    Sphere_3_arr spheres = {((Sphere){8., ((Vec3){-60., 0., -100.}), make_VESA_Colour(255, 0, 0, 255)}), ((Sphere){8., ((Vec3){0., 0., -100.}), make_VESA_Colour(0, 255, 0, 255)}), ((Sphere){8., ((Vec3){60., 0., -100.}), make_VESA_Colour(0, 0, 255, 255)})};
    Parallelepiped_0_arr parallelepipeds = {};
    PerspectiveCamera camera = ((PerspectiveCamera){((Vec3){0., 0., 0.}), ((Vec3){0., 0., -1.}), ((Vec3){0., -1., 0.})});
    LightSource light_source = ((LightSource){((Vec3){5., 5., 0.}), 1., make_VESA_Colour(255, 255, 255, 255)});
    ...
    while (true) {
        for (size_t y = ((size_t)(0)); y < vres; y = ((size_t)(y)) + ((size_t)(1))) {
            for (size_t x = ((size_t)(0)); x < hres; x = ((size_t)(x)) + ((size_t)(1))) {
                ...
                VESA_Colour colour = cast_ray(&ray, spheres, parallelepipeds, 5);
                fb_set_pixel(x, y, colour, get_fb(w));
            }
        }
        set_window_dirty(w);
        juggle(&spheres[1], ticks);
        juggle(&spheres[0], ((int)(ticks)) + ((int)(20)));
        juggle(&spheres[2], ((int)(ticks)) + ((int)(40)));
        sleep(1);
        ticks = ((int)(ticks)) + ((int)(1));
    }
}

void fb_set_pixel(size_t x, size_t y, VESA_Colour colour, Framebuffer* fb) {
